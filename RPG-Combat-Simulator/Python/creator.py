...
import json, re, PySimpleGUI as sg
...
def checar_escudo(arma):
    ...
    if "2-hand" not in arma["props"]:
        ...
        sg.popup(
            "O personagem não pode usar uma arma de 2 mãos com um escudo", title="Erro!"
        )
...
def principal():
    ...
    window = sg.Window(
        ...
        layout=[
            ...
            [
                sg.Text("Arma:", size=(16, 1)),
                sg.InputCombo(weapon_list, key="-WPN-", readonly=True),
            ],
            [
                sg.Text("Armadura:", size=(16, 1)),
                sg.InputCombo(armor_list, key="-ARM-", readonly=True),
            ],
            [
                sg.Text("O personagem tem escudo? ", size=(21, 1)),
                sg.Radio("Sim", "shield", key="-SIM-"),
                sg.Radio("Não", "shield", k="-NAO-"),
            ],
            [sg.Button("Criar personagem"), sg.Button("Fechar")],
        ],
        modal=True,
    )
    # ----------------------------------------------

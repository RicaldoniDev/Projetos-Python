...
def selecione_seu_personagem():
    ...
    window = sg.Window(
        ...
        layout=[
            *[
                [
                    [sg.Radio(f"{char}", "characters", k=f"-NOME_{char}-")],
                    [sg.Text("Arma:"), sg.Text(abrir_json_personagem(char)["weapon"])],
                    [
                        sg.Text("Armadura:"),
                        sg.Text(abrir_json_personagem(char)["armor"]),
                    ],
                    [
                        sg.Text("Escudo:"),
                        sg.Text(abrir_json_personagem(char)["shield"]),
                    ],
                    [
                        sg.Text("Força:"),
                        sg.Text(abrir_json_personagem(char)["strength"]),
                    ],
                    [
                        sg.Text("Destreza:"),
                        sg.Text(abrir_json_personagem(char)["dexterity"]),
                    ],
                    [sg.Text("HP:"), sg.Text(abrir_json_personagem(char)["HP"])],
                    sg.HSep(),
                ]
                for char in nome_pers
            ],  # I've got the screen displaying all characters, but I need to display their info too, and for that I'd need to acess each char file
            [
                sg.Button("Selecionar personagem"),
                sg.Button("Apagar personagem"),
                sg.Button("Sair"),
            ],
        ],
        modal=True,
    ...
    while True:
        ...
        if event == "Selecionar personagem":

from Consumo_mes import Gastos
import PySimpleGUI as sg
import os
import json
import sys

sys.path.insert(0, "/Consumo_mes")
...
class Tela_de_login:
    def __init__(self):
        ...
        with open(
                os.path.join(os.path.abspath(os.path.dirname(__file__)), self.filename), "r+") as save:
            try:
    ...
    def adiciona_user(self, usuario, senha):
        with open(
            ...
            if usuario == "" or senha == "":
                self.popup("Erro!", "Não deixe campos em brnaco")
                ...
                if dados == {}:
                    ...
                    for usuarios in dados:
                        if usuario == usuarios:
                            self.popup(
                                "Aviso!", "Esse nome de usuario já está em uso")
    ...
    def tela_cadastro(self):
        ...
        layout = [
            [
                sg.Text("Digite seu usuário:", p=((0, 20.5), (0, 0))),
                sg.Input(k="cadastro_user", expand_x=True),
            ],
            [
                sg.Text("Digite seu email:", p=((0, 31.5), (0, 0))),
                sg.Input(k="nao_salvar", expand_x=True),
            ],
            [
                sg.Text("Digite sua senha:", p=((0, 27.5), (0, 0))),
                sg.Input(
                    do_not_clear=False,
                    k="cadastro_senha",
                    password_char="*",
            ...
            [
                sg.Text("Confirme sua senha:", p=((0, 10), (0, 0))),
                sg.Input(
                    do_not_clear=False,
                    k="cadastro_confirma",
                    password_char="*",
        ...
        janela_cadastro = sg.Window(
            "Faça seu cadastro", layout=layout, modal=True)
        ...
        while True:
            ...
            if evento == "Cancelar" or evento == sg.WIN_CLOSED:
                ...
                if valores["cadastro_senha"] != valores["cadastro_confirma"]:
                    ...
                    check = self.adiciona_user(
                        valores["cadastro_user"], valores["cadastro_senha"]
                    )
                    if check:
    ...
    def redefinir_senha(self):
        ...
        layout = [
            [
                ...
                sg.Input(k="redefinir_usuario", expand_x=True),
            ],
            [
                sg.Text("Digite sua nova senha:"),
                sg.Input(k="redefinir_senha",
                         password_char="*", expand_x=True),
            ],
            [
                sg.Text("Confirme sua nova senha:"),
                sg.Input(k="redefinir_confirma",
                         password_char="*", expand_x=True),
            ],
                ...
                if valores["redefinir_senha"] != valores["redefinir_confirma"]:
                    ...
                    with open(
                        ...
                        if valores["redefinir_usuario"] not in usuarios:
                            ...
                            valores["redefinir_senha"]
                            == usuarios[valores["redefinir_usuario"]]
                        ):
                            self.popup(
                                "Erro!", "A senha informada já está em uso")

                        else:
                            usuarios[valores["redefinir_usuario"]] = str(
                                valores["redefinir_senha"]
                            )
                            ler_senha.seek(0)
    ...
    def login(self, usuario, senha):
        with open(
            ...
            if usuario not in usuarios:
                self.popup(
                    "Erro!", "Esse usuário não existe, favor realizar o cadastro"
                )
    ...
    def tela_login(self):
        ...
        layout = [
            [
                sg.Text("Digite seu usuário:"),
                sg.Input(do_not_clear=False, k="login_user", s=(30)),
            ],
            [
                sg.Text("Digite sua senha:  "),
                sg.Input(
                    do_not_clear=False, k="login_senha", password_char="*", s=(30)
                ),
            ],
            [
                sg.Button("Login"),
                sg.Button("Cadastrar"),
                sg.Button("Redefinir Senha"),
                sg.Button("Sair"),
            ],
        ]
        ...
        while True:
            ...
            if evento == "Sair" or evento == sg.WIN_CLOSED:
                ...
                check = self.login(
                    valores["login_user"], valores["login_senha"])
                if check:
                    janela_login.close()
                    self.gastos.inicio()
